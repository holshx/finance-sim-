...
export default function ExpensesPage() {
  ...
  const [expenses, setExpenses] = useState<{ amount: string; category: string; type: string; date: string }[]>([]);
  ...
  // Filtered expenses for chart
  const filteredExpenses = expenses.filter(
    (exp) =>
      isInDateRange(exp.date, selectedRange) &&
  ...
  // Ensure chart always renders (Recharts can break if all values are 0 and data is empty)
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto mt-8 px-4">
      <h1 className="text-3xl font-bold mb-6 text-white">Expenses</h1>
      {/* Chart Controls */}
      ...
      <div className="flex flex-col md:flex-row gap-8 w-full">
        <Card className="bg-black text-white border-neutral-800 p-6 flex-1 min-w-[340px]">
          <form className="grid grid-cols-1 gap-4 items-end" onSubmit={handleSubmit}>
            ...
            <div className="flex gap-2 w-full">
              <Button type="submit" className="w-1/2 bg-neutral-800 text-white hover:bg-neutral-700">Add Expense</Button>
        ...
        <Card className="bg-black text-white border-neutral-800 p-6 flex-1 min-w-[340px]">
          <h2 className="text-xl font-semibold mb-4 text-white">Expenses List</h2>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {expenses.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-neutral-400 text-center">No expenses yet.</TableCell>
                </TableRow>
              ) : (
